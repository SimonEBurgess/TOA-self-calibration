function [xopt,yopt]=bundleToaMics2DSounds3D(D,I,J,xt,yt,lambda, debug, weights, maxIter )
...
m = size(x,2);
n = size(y,2);
V = [x(:,I) ; zeros(1,length(I))]-y(:,J); %Just the differences of the positions r_i-s_j, for all combinations
Vt = V';           
...
JJSendX = (J-1)*3+1+2*m;      %Indices of the unknowns which has somethingto do with sender x-coords
JJSendY = (J-1)*3+2+2*m;    %Indices of the unknowns which has somethingto do with sender y-coords
JJSendZ = (J-1)*3+3+2*m;    %Indices of the unknowns which has somethingto do with sender z-coords
%JJ6 = (J-1)*2+3+3*m;    
...
jac = sparse([II;II;II;II;II],[JJrecX;JJrecY;JJSendX;JJSendY;JJSendZ],[VVRecX;VVRecY;VVSendX;VVSendY;VVSendZ],nbrMeas,2*m+3*n);
